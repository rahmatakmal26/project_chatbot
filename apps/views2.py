...
def normalize_text(tokens, normalization_dict, stopwords):
    normalized_tokens = []
    for token in tokens:
        clean_token = re.sub(r'[^a-zA-Z0-9]', '', token).lower()
        if clean_token and clean_token not in stopwords:
            normalized_tokens.append(normalization_dict.get(clean_token, clean_token))
    return normalized_tokens


def normalize_text_sentence(sentence, normalization_dict, stopwords):
...
class ChatbotAPI(APIView):
    def post(self, request):
        # ✅ [TAHAP 1: Terima input dari frontend]
    ...
    def get_response_from_db(self, normalized_tokens):
        ...
        for q in questions:
            ...
            questions_text = normalize_text_sentence(q.questions, normalization_dict, stopwords)


            
            dist = levenshtein_distance(normalized_text, questions_text)
            max_len = max(len(normalized_text), len(questions_text))
            ratio_lev = (1 - dist / max_len) * 100 if max_len > 0 else 0
            ...
            if ratio_fuzz > highest_fuzz_score:
                ...
                best_match = q
                best_ratio_lev = ratio_lev 

        if best_match and highest_fuzz_score > 60:
            response_text = best_match.answers or ""
            print(f"Respons yang dipilih (score={best_ratio_lev}): {response_text}")
